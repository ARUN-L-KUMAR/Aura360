...
interface InvestmentEntryTableProps {
...
export function InvestmentEntryTable({ onTransactionsSaved }: InvestmentEntryTableProps) {
  const { toast } = useToast()
  const [data, setData] = useState<InvestmentRow[]>([createEmptyRow()])
  const [isSavingAll, setIsSavingAll] = useState(false)
  const tableRef = useRef<HTMLDivElement>(null)

  function createEmptyRow(): InvestmentRow {
    return {
  ...
  const validateRow = useCallback((row: InvestmentRow): { isValid: boolean; errors: string[] } => {
    const errors: string[] = []
  ...
  const saveRow = useCallback(
    async (row: InvestmentRow) => {
      ...
      try {
        ...
        toast({
          title: "Success",
          description: "Investment saved successfully",
        })
  ...
  const saveAllRows = useCallback(async () => {
    ...
    if (unsavedRows.length === 0) {
      toast({
        title: "No Changes",
        description: "All valid investment entries are already saved",
      })
      return
    }
    ...
    try {
      ...
      toast({
        title: "Success",
        description: `${unsavedRows.length} investment(s) saved successfully`,
      })
      ...
      toast({
        title: "Save Error",
        description: error.message || "Failed to save investments",
        variant: "destructive",
      })
    } finally {
  ...
  const handlePaste = useCallback(
    async (e: React.ClipboardEvent) => {
      ...
      if (newRows.length > 0) {
        ...
        toast({
          title: "Pasted",
          description: `Added ${newRows.length} investment row(s)`,
        })
      }
    },
    [validateRow, toast]
    ...
    <div className="space-y-4">
      ...
      <div className="text-sm text-muted-foreground bg-purple-50 dark:bg-purple-950/20 border border-purple-200 dark:border-purple-800 rounded-lg p-3">
        <div className="flex items-start gap-2">
          <Upload className="h-4 w-4 mt-0.5 text-purple-600 dark:text-purple-400" />
          <div>
