...
export function ExpenseEntryTable({ onTransactionsSaved }: ExpenseEntryTableProps) {
  const { toast } = useToast()
  const [data, setData] = useState<ExpenseRow[]>([createEmptyRow()])
  ...
  const validateRow = useCallback((row: ExpenseRow): { isValid: boolean; errors: string[] } => {
    const errors: string[] = []
  ...
  const saveAllRows = useCallback(async () => {
    ...
    if (unsavedRows.length === 0) {
      toast({
        title: "No Changes",
        description: "All valid expense entries are already saved",
      })
      return
    }
    ...
    try {
      ...
      if (failures.length > 0) {
        throw new Error(`Failed to save ${failures.length} expense(s)`)
      }
      ...
      toast({
        title: "Success",
        description: `${unsavedRows.length} expense(s) saved successfully`,
      })
  ...
  const handlePaste = useCallback(
    async (e: React.ClipboardEvent) => {
      ...
      if (newRows.length > 0) {
        ...
        toast({
          title: "Pasted",
          description: `Added ${newRows.length} expense row(s)`,
        })
      }
    },
    [validateRow, toast]
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="text-sm text-muted-foreground bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-800 rounded-lg p-3">
        <div className="flex items-start gap-2">
          <Upload className="h-4 w-4 mt-0.5 text-red-600 dark:text-red-400" />
          <div>
